...
{
    // Для роботи з існуючою БД MS SQL Server необхідно додати чотири пакети: 
    // Microsoft.EntityFrameworkCore.SqlServer(представляє функціональність Entity Framework для роботи з MS SQL Server) 
    // Microsoft.EntityFrameworkCore.Tools (пакет для створення класів по базі існуючої бази даних, тобто reverse engineering) 
    // Microsoft.EntityFrameworkCore
    // Microsoft.EntityFrameworkCore.Design

    // Tools->NuGet Package Manager->Package Manager Console
    // Scaffold-DbContext "Server=localhost\SQLEXPRESS;Database=AcademyGroup;Integrated Security=SSPI;TrustServerCertificate=true" Microsoft.EntityFrameworkCore.SqlServer 
    // Scaffold-DbContext "Data Source=AcademyGroup.db" Microsoft.EntityFrameworkCore.Sqlite
    ...
    public partial class Form1 : Form
    {
        ...
        private void RemoveGroupClick(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var db = new AcademyGroupContext())
                {
                    ...
                    MessageBox.Show("Група видалена!");
                }
            }
            catch (Exception ex)
            {
        ...
        private void EditGroupClick(object sender, EventArgs e)
        {
            try
            {
                ...
                using (var db = new AcademyGroupContext())
                {
                    ...
                    MessageBox.Show("Група перейменована!");
                }
            }
            catch (Exception ex)
            {
        ...
        private void AddStudentClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Не вказано ім'я чи прізвище студента!");
                    return;
                }
                if (comboBoxGroup.Items.Count == 0)
                {
